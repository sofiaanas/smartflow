...
AFSAMPLES = ["AF_AFR","AF_AMR","AF_EAS","AF_FIN","AF_NFE","AF_SAS"]
ACSAMPLES = ["AC_AFR","AC_AMR","AC_EAS","AC_FIN","AC_NFE","AC_SAS"]

ANALYSIS = ["merge_exac_af","merge_exac_ac","merge_afackg","merge_nfe_afackg"]
...
rule all:
    input:
        ...
        # samplings
        expand("samplings/ExAC/sample_AF_pop/AF_{sample}.{repetition}.vcf.gz", sample=SAMPLES, repetition=REPETITION),
        expand("samplings/ExAC/sample_AC_pop/AC_{sample}.{repetition}.vcf.gz", sample=SAMPLES, repetition=REPETITION),
        # analysis
        ...
        "analysis/merge_nfe_afackg/smartpca/plotpca.xtxt",
        "analysis/merge_nfe_afackg/smartpca/plotpca.ps",
...
rule sample_AF_pop:
    ...
    output:
        "samplings/ExAC/sample_AF_pop/AF_{sample}.{repetition}.vcf.gz"
    params:
        sample="{sample}",
        rep="{repetition}"
    shell:
        "java -jar {input.jar} -b {input.exac} -bAC AC_{params.sample} -bAN AN_{params.sample} -n AF_{params.sample}.{params.rep} --output {output}"
...
rule sample_AC_pop:
    input:
        ...
        jar= "../simdrom/simdrom-cli/target/simdrom-cli-0.0.3-SNAPSHOT.jar"
    output:
        "samplings/ExAC/sample_AC_pop/AC_{sample}.{repetition}.vcf.gz"
    params:
        sample="{sample}",
        rep="{repetition}"
    shell:
        "java -jar {input.jar} -b {input.exac} -bAChom Hom_{params.sample} -bAChet Het_{params.sample} -bAChemi Hemi_{params.sample} -bAN AN_{params.sample} -n AC_{params.sample}.{params.rep} --output {output}"
...
rule pick_popkg:
    input:
        ...
        pop="pop/nfe.txt"
    output:
        vcf="1KG/1KG.regcut.nfe.vcf.gz",
        idx="1KG/1KG.regcut.nfe.vcf.gz.tbi"
    shell:
        """
        bcftools view -Oz -S {input.pop} {input.vcf} > {output.vcf};
        tabix {output.vcf}
        """
...
rule merge_exac_af:
    input:
        exac=expand("samplings/ExAC/sample_AF_pop/AF_{sample}.{repetition}.vcf.gz", sample=SAMPLES, repetition=REPETITION),
...
rule merge_exac_ac:
    input:
        exac=expand("samplings/ExAC/sample_AC_pop/AC_{sample}.{repetition}.vcf.gz", sample=SAMPLES, repetition=REPETITION),
        kg="1KG/1KG.regcut.vcf.gz"
    output:
...
rule merge_afackg:
    input:
        af=expand("samplings/ExAC/sample_AF_pop/AF_{sample}.{repetition}.vcf.gz", sample=SAMPLES, repetition=REPETITION),
        ac=expand("samplings/ExAC/sample_AC_pop/AC_{sample}.{repetition}.vcf.gz", sample=SAMPLES, repetition=REPETITION),
        kg="1KG/1KG.regcut.vcf.gz"
    output:
...
rule plot_acafkg:
    ...
    params:
        sample=":".join(AFSAMPLES+ACSAMPLES+REAL)
    shell:
        "../../EIG-6.1.4/bin/ploteig -i {input} -c 1:2 -p {params.sample} -x -o {output.xtxt}"
...
rule plot_nfe_acafkg:
    ...
    output:
        xtxt="analysis/merge_nfe_afackg/smartpca/plotpca.xtxt",
        ps="analysis/merge_nfe_afackg/smartpca/plotpca.ps",
        # pdf="smartpca/plotpca.pdf"

    params:
        sample=":".join(NFEREAL)
    shell:
        "../../EIG-6.1.4/bin/ploteig -i {input} -c 1:2 -p AF_NFE:AC_NFE:{params.sample} -x -o {output.xtxt}"
